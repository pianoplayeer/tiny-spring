...
public class AnnotationConfigApplicationContext implements ConfigurableApplicationContext {
	...
	private void disposeBean(Object bean, BeanDefinition def) {
		this.singleObjects.remove(def.getBeanName());
		
		if (def.getDestroyMethod() != null || StringUtils.isNoneBlank(def.getDestroyMethodName())) {
			try {
				Method method = def.getDestroyMethod();
				
				if (method == null) {
					method = bean.getClass().getDeclaredMethod(def.getDestroyMethodName());
					def.setDestroyMethod(method);
				}
				
				method.setAccessible(true);
				method.invoke(bean);
			} catch (ReflectiveOperationException e) {
				throw new BeanInitException(String.format("No destroy method named %s in Bean %s@%s.", def.getInitMethodName(), def.getBeanName(), def.getClazz().getName()));
			}
		}
	}
	...
	public <T> List<T> getBeans(Class<T> requiredType) {
		return beanDefinitionMap.values().stream()
				.filter(def -> requiredType.isAssignableFrom(def.getClazz()))
				.map(def -> (T) getSingleton(def.getBeanName(), false))
	...
	public void close() {
		beanDefinitionMap.values().stream()
				.filter(def -> getSingleton(def.getBeanName(), false) != null)
				.forEach(def -> disposeBean(getSingleton(def.getBeanName(), false), def));
	}
